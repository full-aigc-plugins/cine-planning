# AGENTS.md — PartMe.AI Cine Planning
...
> 本仓是**规划仓**（planning repository），无可安装插件清单，不执行独立 bump；
...
```bash
node scripts/bump-plugin.mjs <宿主插件id> patch   # 文档/注释/小修复
node scripts/bump-plugin.mjs <宿主插件id> minor   # 新功能
node scripts/bump-plugin.mjs <宿主插件id> major   # 破坏性变更
```
